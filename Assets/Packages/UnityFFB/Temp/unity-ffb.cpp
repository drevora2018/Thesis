...
JoystickState GetJoystickAxisData()
{
    JoystickState state = { 0, 0}; // Default to zero
    if (g_pDevice == NULL) {
    ...
    if (FAILED(hr)) {
        ...
        while (hr == DIERR_INPUTLOST) {
            printf("Attempting to aquire device");
            hr = g_pDevice->Acquire();
        }

        if (hr == DIERR_OTHERAPPHASPRIO || hr == DIERR_NOTACQUIRED) {
            printf("Failed to acquire device: Another application has priority or not acquired");
            state.xAxis = .5;
            state.yAxis = -.5;
            return state;
        }
    ...
    DIJOYSTATE2 js;
    if (FAILED(g_pDevice->GetDeviceState(sizeof(DIJOYSTATE), &js))) {
 ...
 */
HRESULT RemoveFFBEffect()
{
   HRESULT hr = E_FAIL;
   Effects::Type effectType = Effects::Type::ConstantForce;
